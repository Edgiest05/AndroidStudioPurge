...
Image GenImageGradientSquare(int width, int height, float density, Color inner, Color outer)
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            // Normalize the distances by the dimensions of the gradient RL_rectangle
...
// Crop an image to area defined by a RL_rectangle
// NOTE: Security checks are performed in case RL_rectangle goes out of bounds
void ImageCrop(Image *image, RL_Rectangle crop)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;

    // Security checks to validate crop RL_rectangle
    ...
    {
        TRACELOG(LOG_WARNING, "IMAGE: Failed to crop, RL_rectangle out of bounds");
        return;
    }
...
#else
    imText = GenImageColor(200, 60, BLACK);     // Generating placeholder black image RL_rectangle
...
// Get image alpha border RL_rectangle
// NOTE: Threshold is defined as a percentage: 0.0f -> 1.0f
...
// Draw RL_rectangle lines within an image
...
void ImageDraw(Image *dst, Image src, RL_Rectangle srcRec, RL_Rectangle dstRec, Color tint)
{
    ...
    else
    {
        ...
        // Source RL_rectangle out-of-bounds security checks
        ...
        // Check if source RL_rectangle needs to be resized to destination RL_rectangle
        ...
        if (((int)srcRec.width != (int)dstRec.width) || ((int)srcRec.height != (int)dstRec.height))
        {
            ...
            ImageResize(&srcMod, (int)dstRec.width, (int)dstRec.height);   // Resize to destination RL_rectangle
        ...
        // Destination RL_rectangle out-of-bounds security checks
        if (dstRec.x < 0)
        {
            srcRec.x -= dstRec.x;
...
// Update GPU texture RL_rectangle with new data
// NOTE: pixels data must match texture.format
...
// Draw a part of a texture (defined by a RL_rectangle)
...
// Draw a part of a texture (defined by a RL_rectangle) with 'pro' parameters
// NOTE: origin is relative to destination RL_rectangle size
